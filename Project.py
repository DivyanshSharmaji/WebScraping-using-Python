...
def find_job_postings(job_title, loc, exp, pages):
    ...
    for page in range(1, pages + 1):
        url = f"{base_url}/job-search/{job_title}-jobs-in-{loc}-{page}?q={job_title}&loc={loc}&minexp={exp}"
        try:
            ...
            for job_card in job_cards:
                try:
                    ...
                    experience = job_soup.find("div", class_="JobDetailWidget_jobCard_lists_item__w6Yow JobDetailWidget_jobIcon__mjaNB undefined").text.strip()
                    ...
                    skills = ""
                    for keyskill in job_soup.find("ul", class_="keyskills_keySkills_items__ej9_3").find_all("a"):
                        skill = keyskill.text.strip()
                        skills = skills + " , " + skill
                    skills = skills.lstrip(" ")
                    skills = skills.lstrip(",")
...
try:
    experience = int(input("Enter number of years experience you have: "))
    ...
    job_postings = find_job_postings(job_title, location, experience, pages)
    if job_postings:
